 ...
 * 静止时表面微流的振幅(uv 单位)。
 *
 * 这是调节"不碰它的时候有多活"的唯一旋钮:波形里各项的权重是固定的,强弱只看这个数。
 * 换算方式是乘画布尺寸:标题在 280 × 162 的画布上时,0.0022 折合横向 1.6px、纵向 0.9px
 * 的峰值位移(典型幅度约 0.8px)。这个量在余光里只是"水面没停",盯着看才分辨得出波形 ——
 * 标题是要给人读的,不是给人盯着看的,所以宁可偏克制。0.0032 起就明显在晃,0.006 以上读作"抖"。
 */
const IDLE_STRENGTH = 0.0022
...
const fragment = /* glsl */ `
  ...
  void main() {
    ...
    // 静止时的表面微流:几组不同频率的波叠加,周期互质所以看不出重复。
    // 没有这一层的话,鼠标一停整个画面就彻底冻住 —— 那不像水,像冰。
    //
    // 分低频与高频两层,不是只把振幅调大。前两组波的空间频率(vUv * 5~9)尺度大于整个单词,
    // 单独放大它们的结果是**整块字一起平移** —— 读出来是"字在抖",不是"水面在流"。
    // 后一组把空间频率提到 13~21,同一个字母的不同部位错开起伏,形变才有剪切感。
    // 权重 0.55 是为了让高频只做质感、不主导轮廓:压到 1.0 的话字边缘会起毛。
    vec2 idle = (
      vec2(
      ...
      + 0.55 * vec2(
        sin(uTime * 1.13 + vUv.x * 21.0 + vUv.y * 13.0),
        cos(uTime * 0.97 + vUv.y * 19.0 - vUv.x * 15.0)
      )
    ) * uIdle;
